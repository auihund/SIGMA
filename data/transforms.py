...
def motion_blur_opencv(image, kernel_size=15, angle=0):
    """
    Apply linear motion blur to an image using OpenCV.

    Args:
        image (PIL.Image): Input image.
        kernel_size (int): Size of the motion blur kernel.
        angle (float): Rotation angle (in degrees) of the motion direction.
    """
    # Create a horizontal linear motion kernel
    ...
    # Normalize the kernel to preserve image intensity
    kernel_sum = rotated_kernel.sum()
    rotated_kernel /= kernel_sum if kernel_sum != 0 else 1

    img = np.array(image)
    if img.ndim == 2:
        # Apply convolution directly for grayscale images
        blurred = cv2.filter2D(
            img, -1, rotated_kernel, borderType=cv2.BORDER_REFLECT
        )
    else:
        # For color images, apply convolution independently per channel
...
def shuffle_patch(image, num_splits, gap_size=2):
    """Split an image into patches (allowing non-divisible sizes),
    randomly shuffle the patches, and reassemble them with gaps in between."""
    ...
    patch_idx = 0          # Index of the current shuffled patch
    for i in range(h_splits):
        current_x = 0      # Starting X coordinate of the current column
        patch_h = patch_heights[i]
        for j in range(w_splits):
            # Retrieve the shuffled patch
            patch = patches[patch_idx]
            patch_w = patch_widths[j]

            # Paste the patch at the current position
            new_image.paste(patch, (current_x, current_y))

            # Move to the next column (patch width + gap)
            current_x += patch_w + gap_size
            patch_idx += 1

        # Move to the next row (patch height + gap)
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """
    Randomly blank out a subset of image patches for inpainting-style training.

    The input image is split into patches, a fraction of patches is replaced
    with a solid-color block, and the image is reassembled to its original size.

    Args:
        image (PIL.Image): Input image in RGB mode.
        num_splits (Tuple[int, int]): Number of splits along (height, width).
        blank_ratio (float): Ratio of patches to be blanked out (0 ~ 1).
        blank_color (Tuple[int, int, int]): RGB color used for blank patches.
    Returns:
        PIL.Image: Reconstructed image with randomly blanked patches.
    """
    h_splits, w_splits = num_splits
    img_w, img_h = image.size
    ...
    # Reconstruct the image to the original resolution
