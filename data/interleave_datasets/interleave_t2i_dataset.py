...
class InterleavedBaseIterableDataset(DistributedIterableDataset):
    ...
    def _add_image(self, data, image, need_loss, need_vae, need_vit, enable_cfg=True, generate=False):
        ...
        if need_loss:
            ...
            if generate == True:
                image_tensor = self.generate_transform(image)
            else:
...
class ParquetStandardIterableDataset(DistributedIterableDataset):
    ...
    def __init__(
        ...
        local_rank=0, world_size=1, num_workers=8, data_status=None, generate_transform=None,
    ):
        """
        ...
        self.generate_transform = generate_transform
