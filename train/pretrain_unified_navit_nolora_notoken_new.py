...
import sys
sys.path.append("/fi-lib/workspace/zxy/code/Bagel_lora")
...
class ModelArguments:
    ...
    use_mask_attn: bool = field(
        default=False,
        metadata={"help": "Use attention mask for <special token>text<image>."}
    )
...
def main():
    ...
    train_dataset = PackedDataset(
        ...
        use_mask_attn=model_args.use_mask_attn,
    )
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        # for img_idx in range(data['padded_images'].shape[0]):
        #     save_image(data['padded_images'][img_idx], f"data/all_8k_multi_condition_data/debug_img/data_{curr_step}_{device}_{img_idx}_{train_dataset.tokenizer.decode(data['packed_text_ids'], skip_special_tokens=True)[:100]}.png")
        # print(f"generated data_{curr_step}_{device} shape: {data['padded_images'][-1].shape}")
        ...
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            if training_args.visual_gen:
                with torch.no_grad():
                    ...
                    imgs = data.pop('padded_images', None)
                    if imgs is not None:
                        with torch.no_grad():
                            data['padded_latent'] = vae_model.encode(imgs)
                    else:
                        logger.info(f"skip sample at {train_step}...")
                        continue
                        data['padded_latent'] = None  # 或者直接不传这个键
