...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        if train_steps % 2000 == 0:
            ...
            save_path = os.path.join(ckpt_dir, f"final_step")
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        ...
        print(f"ema_model type: {type(ema_model)}")
        print(f"resume_from_ema: {resume_from_ema}")
        keys_to_remove = [
            'latent_pos_embed.pos_embed',
            'vit_pos_embed.pos_embed',
            'base_model.model.latent_pos_embed.pos_embed',
            'base_model.model.vit_pos_embed.pos_embed'
        ]
        if resume_from is not None and os.path.exists(resume_from):
            logger.info(f"Loading checkpoint from {resume_from}.")
            if resume_from_ema:
                model_state_dict_path = os.path.join(resume_from, f"ema_lora.safetensors")
            else:
            ...
            for key_pattern in keys_to_remove:
                model_state_dict.pop(key_pattern, None)
            ...
            if ema_model is not None:
                if "Mot" in resume_from:
                    ema_state_dict_path = os.path.join(resume_from, f"ema_lora.safetensors")
                else:
                    ema_state_dict_path = os.path.join(resume_from, f"ema.safetensors")
                print(f"ema_state_dict_path: {ema_state_dict_path}")
                ...
                for key_pattern in keys_to_remove:
                    ema_state_dict.pop(key_pattern, None)
    ...
    @staticmethod
    def try_load_ckpt_all_params(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            logger.info(f"Loading checkpoint from {resume_from}.")
            if resume_from_ema:
